...
class TestCheck {
  ...
  Future getDataBusstop() async {
    ...
    for (var i = 0; i < latLng.length; i++) {
      var lat = double.parse(busstop[15].sLongitude);
      var lng = double.parse(busstop[15].sLatitude);
      if ((lat >= (latLng[i].latitude - 0.0006) &&
              lat <= (latLng[i].latitude + 0.0006)) &&
          (lng >= (latLng[i].longitude - 0.0006) &&
              lng <= (latLng[i].longitude + 0.0006))) {
        print('Check point Sid : ' + i.toString());
      }
    }
    //setState(() {});
  }

  String checkLatlnt(LocationData data) {
    String result = 'defalut';
    for (var i = 0; i < latLng.length; i++) {
      if ((data.latitude >= (latLng[i].latitude - 0.0006) &&
              data.latitude <= (latLng[i].latitude + 0.0006)) &&
          (data.longitude >= (latLng[i].longitude - 0.0006) &&
              data.longitude <= (latLng[i].longitude + 0.0006))) {
        print('Check point Sid : ' + i.toString());
        result = 'inrange';
        break;
