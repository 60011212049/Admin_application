...
class AddStatusWork extends StatefulWidget {
  @override
  _AddStatusWorkState createState() => _AddStatusWorkState();
}

class _AddStatusWorkState extends State<AddStatusWork> {
  var status = {};
  ...
  Future<Null> getBusstop() async {
    ...
    // selectIn = busstop[0].sName;
    // selectOut = busstop[0].sName;
    setState(() {});
    return null;
  }
  ...
  Future<Null> addTransciption() async {
    ...
    status['type'] = 'เพิ่มสถานะเข้าออกงานคนขับ ' + _nameIdcontroller.text;
  ...
  Future<Null> _sentDataStatusWork() async {
    ...
    status['id'] = _nameIdcontroller.text;
    String jsonSt = json.encode(status);
    var response = await http.post(
